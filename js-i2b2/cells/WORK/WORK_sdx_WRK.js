...
i2b2.sdx.TypeControllers.WRK.onHoverOver = function(e, id, ddProxy) {    
	try
	{
		var el = $(id);	
		if (el) { 
			var folderObject = false;
			if(draggedEl.className == 'wrkFolder' || draggedEl.className == 'wrkRoot')
				folderObject = true;
			else
			{
				var folderChildObject = jQuery(draggedEl).find('.wrkRoot');
				if(folderChildObject && folderChildObject.length>0)
					folderObject = true;
				folderChildObject = jQuery(draggedEl).find('.wrkFolder');
				if(folderChildObject && folderChildObject.length>0)
					folderObject = true;
			}
			if(folderObject)
				Element.addClassName(el,'ddWRKTarget'); 
		}
	}
	catch(e){}
}
